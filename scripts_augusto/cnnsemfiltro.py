...
Original file is located at
    https://colab.research.google.com/drive/1ZeikXK5faYdBB5sg59tG2RbqwgFaKtz8
...
def normalizer(signal):
  for cont_sin in range(len(signal)):
    ...
    desvio = np.std(signal)
    result = (signal-media)/desvio
...
#Realizando a normalização do sinal
temponorm1 = time.time()
mlii = df["MLII"].values
pos = df["Temp"].values

mlii_normalized = normalizer(mlii)

##Adequando posições dos picos R
#for index in range(len(r_peak)):
#  if(r_peak[index]+20 < len(mlii_normalized)):
#    r_peak[index] = r_peak[index]+20
#  else:
#    r_peak.pop(index)

df["MLII"] = mlii_normalized
df["Temp"] = pos
temponorm2 = time.time()
temponorm = temponorm2-temponorm1

#Dividir por batida (Segmentação)
temposegm1 = time.time()
pnts_ant = 130
pnts_post = 130
...
temposegm2 = time.time()
temposegm = temposegm2-temposegm1
...
tempocria1 = time.time()
model = criar_modelo()

model.load_weights("SEMFILTROmodelo_CNN.h5")
tempocria2 = time.time()
tempocria = tempocria2-tempocria1
#Aplicar a classificação da batida através do modelo carregado
tempopredic1 = time.time()
predic = model.predict([base_MLII])
tempopredic2 = time.time()
tempopredic = tempopredic2-tempopredic1
#Transformar a predição em um vetor de saída
#Identificando batida através de predição
tempotrad1 = time.time()
vetor_final_predic = tpSig_predc(predic)
tempotrad2 = time.time()
tempotrad = tempotrad2-tempotrad1
tempototal2 = time.time()
tempototal = tempototal2-tempototal1

print(vetor_final_predic)

print("Tempo de importação: ", tempoimport)
print("Tempo de normalização: ", temponorm)
print("Tempo de segmentação: ", temposegm)
print("Tempo de criação do modelo: ", tempocria)
print("Tempo de predição: ", tempopredic)
print("Tempo de tradução: ", tempotrad)
